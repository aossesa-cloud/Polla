...
import { getAvailableNavItems } from '../config/routes'
...
const navItems = [
  ...
  { id: 'campanas', label: 'Campañas', icon: 'campanas' },
  { id: 'calendario', label: 'Calendario', icon: 'calendario' },
  { id: 'programa', label: 'Programa', icon: 'programa' },
  { id: 'resultados', label: 'Resultados', icon: 'resultados' },
  { id: 'ingreso', label: 'Ingreso Picks', icon: 'ingreso' },
...
export default function Sidebar({ activeView, onNavigate, user, onLogout, onShowLogin }) {
  // Obtener items de navegación disponibles según estado de auth
  const availableNavItems = getAvailableNavItems(!!user)
    ...
    <aside className={styles.sidebar}>
      <div className={styles.logo}>
        ...
        <div className={styles.logoText}>
          <span className={styles.logoTitle}>Polla Hípica</span>
          <span className={styles.logoSubtitle}>
            {user ? 'Panel de Control' : 'Vista Pública'}
          </span>
        </div>
      </div>
      ...
      {user && (
        <div className={styles.userSection}>
          <div className={styles.userInfo}>
            <span className={styles.userAvatar}>{user.displayName?.[0] || user.username?.[0] || 'A'}</span>
      ...
      {!user && onShowLogin && (
        <div className={styles.publicSection}>
          <div className={styles.publicNotice}>
            ...
            <span className={styles.noticeText}>Vista pública</span>
          </div>
